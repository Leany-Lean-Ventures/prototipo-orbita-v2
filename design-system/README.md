...
| `logos/` | **Marca oficial** — 6 variações (horizontal/vertical/favicon × positivo/negativo) em SVG + PNG, extraídas de `brand-book-ademicon.svg`. Folha de referência: `logos/index.html`. Ver §Marca. | asset real |
...
Set oficial em `logos/`, extraído de `brand-book-ademicon.svg`. Cada variação vem em **SVG** (fonte vetorial, recorte exato do artboard via `viewBox`) e **PNG** (conveniência). Folha de referência visual: **`logos/index.html`**.

| Arquivo (base) | Variação | Uso |
| --- | --- | --- |
| `ademicon-logo-horizontal-color` | Horizontal · positivo | assinatura principal, fundo claro |
| `ademicon-logo-horizontal-white` | Horizontal · negativo | fundo escuro / foto com overlay |
| `ademicon-logo-vertical-color` | Vertical · positivo | lockup empilhado, fundo claro |
| `ademicon-logo-vertical-white` | Vertical · negativo | lockup empilhado, fundo escuro |
| `ademicon-favicon` | Favicon · positivo | símbolo isolado (aba/app), fundo claro |
| `ademicon-favicon-white` | Favicon · negativo | símbolo isolado, fundo escuro/vermelho |

Regras: **positivo** em fundo claro, **negativo** (branco) em fundo escuro ou foto com overlay ≥4,5:1; cor da marca `#dc2626` — não recolorir, distorcer nem aplicar glow; favicon = só o símbolo; respeitar área de proteção. `logos/ademicon-logo.png` é o export legado do símbolo (mantido por compatibilidade). Preservar os arquivos reais — não redesenhar.
